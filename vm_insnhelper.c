                          ...
                          int argc, VALUE *argv, VALUE *stack_bottom)
{
    ...
    EXEC_EVENT_HOOK_B(ec, RUBY_EVENT_C_CALL, recv, me->def->original_id, vm_ci_mid(ci), me->owner, me->def->box, Qundef);
    ...
    EXEC_EVENT_HOOK_B(ec, RUBY_EVENT_C_RETURN, recv, me->def->original_id, vm_ci_mid(ci), me->owner, me->def->box, val);
...
#define VM_CALL_METHOD_ATTR(var, func, nohook) \
    if (UNLIKELY(ruby_vm_c_events_enabled > 0)) { \
        EXEC_EVENT_HOOK_B(ec, RUBY_EVENT_C_CALL, calling->recv, vm_cc_cme(cc)->def->original_id, \
                        vm_ci_mid(ci), vm_cc_cme(cc)->owner, vm_cc_cme(cc)->def->box, Qundef); \
        var = func; \
        EXEC_EVENT_HOOK_B(ec, RUBY_EVENT_C_RETURN, calling->recv, vm_cc_cme(cc)->def->original_id, \
              ...
              rb_hook_list_t *global_hooks, rb_hook_list_t *local_hooks, VALUE val)
{
    ...
    if (event & global_hooks->events) {
        ...
        reg_cfp->pc++;
        vm_dtrace(event, ec);
        rb_exec_event_hook_orig(ec, global_hooks, event, self, 0, 0, 0 , val, NULL, 0);
        reg_cfp->pc--;
    }
    ...
    if (local_hooks != NULL) {
        if (event & local_hooks->events) {
            /* increment PC because source line is calculated with PC-1 */
            reg_cfp->pc++;
            rb_exec_event_hook_orig(ec, local_hooks, event, self, 0, 0, 0 , val, NULL, 0);
            reg_cfp->pc--;
        }
    }
}
